...
def main(args):
    ...
    if args.mlflow_uri:
        # Respect whatever the user passes: either a plain path or a file URI
        mlflow.set_tracking_uri(args.mlflow_uri)
    else:
        # Use a local ./mlruns folder with a correct file:// URI on any OS
        tracking_dir = (PROJECT_ROOT / "mlruns").resolve()
        tracking_dir.mkdir(parents=True, exist_ok=True)
        mlflow.set_tracking_uri(tracking_dir.as_uri())  # e.g., file:///C:/... on Windows
...
if __name__ == "__main__":
    ...
    p.add_argument("--mlflow_uri", type=str, default=None,
                   help="override MLflow tracking URI; accepts plain path or file URI (file:///C:/...)")

    args = p.parse_args()
    main(args)

"""
# Use this below to run the pipeline (PowerShell uses backticks for line breaks):
